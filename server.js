        ...
        await db.tx(async t => {
            const patientId = await t.one(`
                INSERT INTO patient (
                    ...
                    weight, height, waist, password, age
                ) VALUES (
                    $[title_name], $[first_name], $[last_name], $[id_card], $[phone], $[gender], $[date_birth],
                    $[house_number], $[street], $[village], $[subdistrict], $[district], $[province],
                    $[weight], $[height], $[waist], $[password], $[age]
...
app.get('/remaining-queue/:program_name/:appointment_date', async (req, res) => {
    const { program_name, appointment_date } = req.params;

    try {
        // ดึงจำนวนคิวสูงสุดของโปรแกรมที่เลือก
        const maxQueue = await db.one('SELECT max_queue FROM appointment_limits WHERE program_name = $1', [program_name]);
        
        // นับจำนวนการนัดหมายในวันที่เลือกและโปรแกรมที่เลือก
        const currentQueue = await db.one('SELECT COUNT(*) FROM appointments WHERE program_name = $1 AND appointment_date = $2', [program_name, appointment_date]);

        // คำนวณจำนวนคิวที่เหลือ
        const remainingQueue = maxQueue.max_queue - parseInt(currentQueue.count);

        res.status(200).json({ remainingQueue });
    } catch (err) {
        console.error('Error fetching remaining queue:', err);
        res.status(500).json({ message: 'Internal Server Error', error: err.message });
    }
});

app.put('/queue-limit/:program_name', async (req, res) => {
    const { program_name } = req.params;
    const { maxQueue } = req.body;
  
    try {
      await db.none(
        'UPDATE appointment_limits SET max_queue = $1 WHERE program_name = $2',
        [maxQueue, program_name]
      );
  
      res.status(200).json({ message: 'Queue limit updated successfully' });
    } catch (err) {
      console.error('Error updating queue limit:', err);
      res.status(500).json({ message: 'Internal Server Error', error: err.message });
    }
  });
  
  app.get('/queue-limits', async (req, res) => {
    try {
      const limits = await db.any('SELECT program_name, max_queue FROM appointment_limits');
      res.status(200).json(limits);
    } catch (err) {
      console.error('Error fetching queue limits:', err);
      res.status(500).json({ message: 'Internal Server Error', error: err.message });
    }
  });
